<template>
  <div>
    <v-stepper v-model="step" vertical elevation="0" class="medico-stepper">
      <!-- Dados Pessoais -->
      ...
      <v-stepper-content step="1">
        <v-form ref="formPt1" lazy-validation>
          ...
          <v-row>
            <v-col cols="12" :xs="12" :sm="6" :md="4">
              <v-text-field
                ...
                :rules="[
                  v => !!v || 'Telefone Celular obrigatório',
          ...
          <v-row>
            <v-col>
              <v-btn color="accent" @click="proximoPasso('formPt1', 2)">
                Continuar
              </v-btn>
            </v-col>
          </v-row>
      ...
      <v-stepper-content step="2">
        <v-form ref="formPt2" lazy-validation>
          ...
          <v-row>
            <v-col>
              ...
              <v-btn color="accent" @click="proximoPasso('formPt2', 3)">
                Continuar
              </v-btn>
            </v-col>
          </v-row>
      ...
      <!--  Dados Acadêmicos -->
      <v-stepper-step :complete="step > 3" color="#007970" step="3">
        Dados Acadêmicos
      </v-stepper-step>

      <v-stepper-content step="3">
        <!-- Escolaridade, Sociedade Cientifica -->
        <v-form ref="formPt3" lazy-validation>
          <v-row>
            <v-col cols="12" :xs="12" :md="5">
              <v-select
                :items="[
              ...
              >
                <template v-slot:append>
                  <v-tooltip bottom>
                    ...
                    Associação de especialistas, estudiosos ou eruditos de um
                    ramo do conhecimento ou das ciências em geral.
                  </v-tooltip>
                </template>
              </v-text-field>
            </v-col>
                  ...
                  <v-row>
                    ...
                    <v-col cols="12" :xs="12" :sm="6" :md="3">
                      <v-text-field
                        ...
                        v-model="
                          formData.formacao[fidx].faculdade_ano_formatura
                        "
                        @blur="salvarEmCache"
                      />
                    </v-col>
                    <v-col
                      ...
                      v-if="fidx > 0"
                      @click="
                        formData.formacao.splice(fidx, 1);
                        salvarEmCache();
                      "
                    >
          ...
          <v-row>
            <v-col>
              ...
              <v-btn color="accent" @click="proximoPasso('formPt3', 4)">
                Continuar
              </v-btn>
            </v-col>
          </v-row>
      ...
      <v-stepper-content step="4">
        <v-form ref="formPt4" lazy-validation>
          <!-- CRM, Categoria -->
          <v-row>
            <v-col cols="12" :xs="12" :md="3">
              <v-text-field
              ...
              >
                <template v-slot:activator="{ on, attrs }">
                  <v-text-field
                    ...
                    class="medico-stepper-input-date"
                    :rules="[
                      v => !!v || 'Data de Inscrição do CRM obrigatório'
                    ]"
                  >
                    <span slot="append">
                      <v-icon v-bind="attrs" v-on="on">
                ...
                <li v-for="(espe, eidx) in formData.especialidade" :key="eidx">
                  <v-row>
                    ...
                    <v-col
                      ...
                      :md="1"
                      @click="
                        formData.especialidade.splice(eidx, 1);
                        salvarEmCache();
                      "
                    >
                      <v-btn icon class="mt-3">
          ...
          <v-row>
            <v-col>
              <v-btn text @click="adicionarEspecialidade">
                Adicionar
                {{ formData.especialidade.length > 0 ? "outra" : "" }}
                especialidade
              </v-btn>
            </v-col>
          </v-row>
          ...
          <v-row>
            <v-col>
              ...
              <v-btn color="accent" @click="proximoPasso('formPt4', 5)">
                Continuar
              </v-btn>
            </v-col>
          </v-row>
      ...
      <v-stepper-content step="5">
        ...
        <v-row>
          <v-col>
            <small class="text-muted">
              ...
              vitae egestas leo. Nam faucibus non lorem sit amet porta.
            </small>
            <a href="/documentos.zip" download class="d-block"
              >Link de Download</a
            >
          </v-col>
        </v-row>
        ...
        <v-row>
          <v-col>
            ...
            <v-btn color="accent" @click="enviarDados">
              Concluir
            </v-btn>
          </v-col>
        </v-row>
...
export default {
  ...
  mounted(){//mexer aqui para fazer o get correto do medico!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  methods: {
    ...
    async getMedicoParaAtualizar(id){
        await app.$axios.get(`/medico/${id}`).then(res => {
        data.unidades = res.data})
        .catch(err => {
            console.log(err.response)
        })
